...
import { Trophy, RefreshCcw, ArrowLeft, ArrowRight, Calendar, Clock, GraduationCap, Play, Pause, Volume2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      {comments.map(comment => (
        <div key={comment.id} className={`p-4 rounded-lg ${isDark ? 'bg-gray-700/50' : 'bg-gray-100'}`}>
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center mt-2">
                <button className={`flex items-center text-xs ${isDark ? 'text-gray-400 hover:text-purple-400' : 'text-gray-500 hover:text-purple-600'}`}>
        ...
        <div className={`p-4 rounded-xl shadow-lg ${
          ...
          {hasSubmitted ? (
            <div className="text-center py-2">
              <p className={`${isDark ? 'text-teal-400' : 'text-teal-600'} font-medium`}>
        ...
        <div key={i} className="animate-pulse">
          <div className={`p-6 rounded-xl border-2 ${
            ...
            <div className="flex items-center mb-4">
              <div className={`w-12 h-12 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-300'}`} />
              <div className="ml-4 flex-1">
                <div className={`h-4 ${isDark ? 'bg-gray-700' : 'bg-gray-300'} rounded w-1/3`} />
                <div className={`h-3 ${isDark ? 'bg-gray-700' : 'bg-gray-300'} rounded w-1/4 mt-2`} />
              </div>
            </div>
            <div className={`h-4 ${isDark ? 'bg-gray-700' : 'bg-gray-300'} rounded w-3/4 mb-4`} />
            <div className={`h-2 ${isDark ? 'bg-gray-700' : 'bg-gray-300'} rounded-full`} />
          </div>
        </div>
      ))}
    </div>
...
export const Education: React.FC<EducationProps> = ({
  ...
  const [celebrationType, setCelebrationType] = useState<'kitten' | 'curious' | 'wise' | null>(null);
  const [activeUnitId, setActiveUnitId] = useState<number | null>(null);
  const [activeModuleGroup, setActiveModuleGroup] = useState<'intro' | 'core' | 'advanced'>('intro');
  ...
  const handleQuizComplete = (unitId: number, score: number) => {
    ...
    const unit = data?.moduleGroups.intro.units.find(u => u.id === unitId) || 
    ...
    if (data) {
      ...
      Object.keys(updatedModuleGroups).forEach(groupKey => {
        const group = updatedModuleGroups[groupKey as keyof typeof updatedModuleGroups];
      ...
      const totalCompleted = Object.values(updatedModuleGroups).reduce(
        (sum, group) => sum + group.units.filter(u => u.status === 'completed').length,
        0
      );
      
      // Update user XP
      const earnedXP = score >= 70 ? unit?.quiz.reward.xp || 0 : Math.floor((unit?.quiz.reward.xp || 0) / 2);
  ...
  const handleFeedbackSubmit = (unitId: number, feedback: 'happy' | 'neutral' | 'sad') => {
  ...
  const handleModuleGroupChange = (group: 'intro' | 'core' | 'advanced') => {
    ...
    if (data?.moduleGroups[group].unlockRequirement) {
      const completedUnits = Object.values(data.moduleGroups).reduce(
        (sum, g) => sum + g.units.filter(u => u.status === 'completed').length,
        0
      );
  ...
  const handleTouchEnd = () => {
    if (touchStart - touchEnd > 75) {
      ...
      if (activeModuleGroup === 'intro' && data?.moduleGroups.core && 
          data.totalProgress.completed >= (data.moduleGroups.core.unlockRequirement || 0)) {
        handleModuleGroupChange('core');
      } else if (activeModuleGroup === 'core' && data?.moduleGroups.advanced && 
                data.totalProgress.completed >= (data.moduleGroups.advanced.unlockRequirement || 0)) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4">
      {/* Progress Header */}
      <div className={`mb-8 p-4 sm:p-6 rounded-xl ${
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center">
              <GraduationCap className={`w-5 h-5 mr-2 ${isDark ? 'text-purple-400' : 'text-purple-600'}`} />
        ...
        <div className={`mt-4 h-2 rounded-full ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`}>
          <div
            ...
            style={{
              width: data
                ? `${(data.totalProgress.completed / data.totalProgress.total) * 100}%`
                : '0%'
            }}
          />
        </div>
      ...
      <div className="mb-8 flex flex-wrap gap-2"
        ...
        <button
          ...
          disabled={data?.totalProgress.completed < (data?.moduleGroups.core.unlockRequirement || 0)}
          className={`px-4 py-2 rounded-lg transition-colors ${
        ...
        <button
          ...
          disabled={data?.totalProgress.completed < (data?.moduleGroups.advanced.unlockRequirement || 0)}
          className={`px-4 py-2 rounded-lg transition-colors ${
        ...
        >
          <span className="flex items-center">
            ...
            {data?.totalProgress.completed < (data?.moduleGroups.advanced.unlockRequirement || 0) && (
              <span className="ml-2 text-xs">
                (Unlock at {data?.moduleGroups.advanced.unlockRequirement} units)
              </span>
            )}
          </span>
        </button>
              ...
              {activeUnit ? (
                  ...
                  <div className="lg:col-span-2">
                    <div className={`p-4 sm:p-6 rounded-xl ${
                      ...
                      <div className="space-y-6">
                            ...
                            <div className="aspect-video bg-black flex items-center justify-center">
                              <div className="text-center p-4">
                                ...
                                <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                  Video placeholder - In a real app, this would play the video from {activeUnit.content.video}
                                </p>
                              </div>
                            </div>
                          </div>
                        ...
                        {activeUnit.content.infographic && !activeUnit.content.video && (
                          <div className="mt-6 rounded-lg overflow-hidden">
                            <img 
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                              ...
                              <div className="flex items-center mt-2 sm:mt-0">
                                ...
                                <span className={`text-sm font-medium ${
                                  ...
                                  {activeUnit.quiz.reward.badge && ` + ${activeUnit.quiz.reward.badge} Badge`}
                                </span>
                              </div>
                            </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-6">
                    ...
                    {data?.moduleGroups[activeModuleGroup].units.map((unit, index) => (
                      <UnitCard
                        key={unit.id}
                        unit={unit}
                        isLast={index === data.moduleGroups[activeModuleGroup].units.length - 1}
                        onStart={() => handleUnitSelect(unit.id)}
                        onQuizComplete={(unitId, score) => handleQuizComplete(unitId, score)}
